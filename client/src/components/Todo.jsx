...
export function Todo({ darkTheme, todo, isArchived }) {
    ...
    return (
        <li key={todo.todo_id} className={`${styles} list-group-item d-flex align-items-center justify-content-between my-1 px-2 border border-secondary`} id={`todo${todo.todo_id}`} >
            <p className={`h6 px-1  `}>
                {todo.description}
            </p>
            {isArchived ?
