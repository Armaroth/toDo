...
export function ArchivedTodosModalContent({ todos, status }) {
    ...
    if (status === 'pending' || todos?.length && todos[0].user_id !== currentUser.id) return <li key={'loading'} className="list-group-item d-flex justify-content-between my-1 px-2 border border-secondary" >
        <h2 className='h6'>Loading</h2>
    </li>

    if (status === 'success' && !todos?.length) return <li key={'empty'} className="list-group-item d-flex justify-content-between my-1 px-2 border border-secondary" >
