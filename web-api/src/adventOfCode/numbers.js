const x = `Game 1: 6 green, 3 blue; 3 red, 1 green; 4 green, 3 red, 5 blue
...
for (let turn of game) {
    turnsArray.push(turn.split(':')[1])
    turnNo.push(turn.split(':')[0].split(' ')[1])
}
const processedTurns = [];

// Iterate through each turn in the input array
turnsArray.forEach(turnString => {
    // Split the turnString on ';'
    const turns = turnString.trim().split(';');
    // console.log(turns);
    // Initialize an array to store the counts for each color in the turn
    const turnCounts = [];

    // Iterate through each turn in the current turnString
    turns.forEach(turn => {
        // Split each turn on ','
        const balls = turn.trim().split(', ');

        // Initialize an object to store the count and color for each turn
        const turnInfo = {};

        // Iterate through each ball in the turn
        balls.forEach(ball => {
            // Split each ball on ' ' to get count and color
            const [count, color] = ball.split(' ');

            // Store count and color in the turnInfo object
            turnInfo[color] = parseInt(count, 10);
        });

        // Push the turnInfo object to the turnCounts array
        turnCounts.push(turnInfo);
    });

    // Push the turnCounts array to the processedTurns array
    processedTurns.push(turnCounts);
});
for (let index = 0; index < processedTurns.length; index++) {
    isLegit[index] = true;
}
for (let index = 0; index < isLegit.length; index++) {
    leastCubes.push({ red: 0, green: 0, blue: 0 })
}
for (let i = 0; i < processedTurns.length; i++) {

    for (let j = 0; j < processedTurns[i].length; j++) {
        if (!processedTurns[i][j].red) processedTurns[i][j].red = 0
        if (!processedTurns[i][j].blue) processedTurns[i][j].blue = 0
        if (!processedTurns[i][j].green) processedTurns[i][j].green = 0

        if (processedTurns[i][j].red > leastCubes[i].red) {
            leastCubes[i].red = processedTurns[i][j].red
        }
        if (processedTurns[i][j].blue > leastCubes[i].blue) {
            leastCubes[i].blue = processedTurns[i][j].blue
        }
        if (processedTurns[i][j].green > leastCubes[i].green) {
            leastCubes[i].green = processedTurns[i][j].green
        }
    }
}
for (let index = 0; index < isLegit.length; index++) {

    powers[index] = leastCubes[index].red * leastCubes[index].green * leastCubes[index].blue;
}
for (let index = 0; index < isLegit.length; index++) {
    sum = sum + powers[index]
}
console.log(sum)
